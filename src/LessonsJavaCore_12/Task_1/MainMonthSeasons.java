...
public class MainMonthSeasons {
    public static void main(String[] args) {
        ...
        List<SeasonsUa> seasonsList = new ArrayList<>();
        for (SeasonsUa valueSeasons : SeasonsUa.values()) {
        ...
        switch (value) {
            ...
            case 5:{
                ...
                if(monthDay == 31 || monthDay < 31 && monthDay >= 28){
                    ...
                    print("Ведене число невірне  ");
                break;
            }
            case 6:{
                print("Ведіть пору року");
                String seasons = scanner.next();
                SeasonsUa seasons1 = SeasonsUa.valueOf(upperCase(seasons.toLowerCase()));
    ...
    static void printMonthDay(int specifiedNumber){
        for (MonthUa monthUa : MonthUa.values()){
            if (monthUa.getDays() == specifiedNumber){
                print(monthUa.name() + " " + monthUa.getDays());
            }
        }
    }
